...
<section class="city-neighborhoods" id="neighborhoods">
    <div class="city-neighborhoods__inner">
        <h2 class="city-neighborhoods__heading">Learn About Neighborhoods Within <?php echo esc_html( $city_name ); ?> CA</h2>
        <div class="city-neighborhoods__grid">
          <?php foreach ( $neighborhoods as $hood ) :
    $hood_link = isset( $hood['neighborhood_link'] ) ? $hood['neighborhood_link'] : '';
?>
    <div class="city-neighborhoods__card">
        <h3 class="city-neighborhoods__card-title">
            <?php if ( $hood_link ) : ?>
                <a href="<?php echo esc_url( $hood_link ); ?>" class="city-neighborhoods__card-link">
                    <?php echo esc_html( $hood['neighborhood_name'] ); ?>
                </a>
            <?php else : ?>
                <?php echo esc_html( $hood['neighborhood_name'] ); ?>
            <?php endif; ?>
        </h3>
        <p class="city-neighborhoods__card-desc"><?php echo esc_html( $hood['neighborhood_description'] ); ?></p>
    </div>
<?php endforeach; ?>
        </div>
    </div>
